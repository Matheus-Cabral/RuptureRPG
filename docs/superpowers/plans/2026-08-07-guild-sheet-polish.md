...
> **Why this is correct:** child mutations don't bump the guild's `xmin`, so when the blob is clean, `refreshed.Data.Version` equals the current guild Version anyway (adoption is a no-op) UNLESS a concurrent user saved the blob — in which case, clean-blob adoption is safe (nothing local to lose). When the blob is dirty and a concurrent save moved the Version, NOT adopting keeps the pre-move Version so the next Save 409s (the protection the whole xmin design exists for). The server 409 path is already tested (`GuildUpdateTests` cross-request conflict).
