...
**Catalog schema field-name contract (`CatalogSchema.cs`):**
- `CatalogSchema.FieldsFor(type)` drives the GM Catalog admin page's structured (non-advanced) form generically — `GmCatalog.razor` builds one input per `CatalogField` and serializes them by `Key` into `DataJson`. That `Key` **must exactly match the corresponding `Catalog*CatalogData` class's C# property name** (e.g. `EquipmentItemCatalogData`), because that class — not the schema — is what services like `CharacterStatsCalculator` actually `JsonSerializer.Deserialize` the saved `DataJson` into. The schema and the data class are two independent files with no compiler link between them.
- This exact mismatch shipped once for `EquipmentItem`: the schema listed `Damage`/`Defense`/`Properties`/`Description`, none of which exist on `EquipmentItemCatalogData` (`Rarity`, `AttackBonus`, `DamageBonus`, `DefenseBonus`, `WeaponDiceCategory`, `ArmorDamageReduction`). Every equipment item created through the form silently deserialized with those fields at their C# defaults — equipment contributed 0 to NP (no `Rarity`), 0 to Damage Reduction (no `ArmorDamageReduction`), and nothing to weapon Attack/Damage math (no `AttackBonus`/`DamageBonus`) — with no error anywhere, since `SafeDeserialize` treats a shape mismatch the same as a missing entry. Fixed by aligning the schema's `EquipmentItem` field list field-for-field with the data class. When adding/renaming a field on any `Catalog*CatalogData` class, update `CatalogSchema.cs`'s matching entry in the same change, and vice versa.
- Related, narrower gap: several `Catalog*CatalogData` string fields (e.g. `EquipmentItemCatalogData.Category`) are free text with no dropdown (`CatalogFieldKind` has no "select" variant) but are compared against fixed lowercase values in calculators — `CharacterStatsCalculator.CategoryIs` compares case/whitespace-insensitively for this reason, but any *new* enum-like free-text catalog field being read by exact `==` elsewhere should do the same rather than trust GM-typed casing.
