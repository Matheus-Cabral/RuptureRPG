...
// ErrorCodes.Content must resolve to a real value in BOTH the English (neutral) and pt-BR resources.
// Mirrors CombatErrorCodeLocalizationTests — a ResourceManager bound to the SharedResources embedded
...
public class ContentErrorCodeLocalizationTests
{
    private static readonly ResourceManager Resources =
        new("Ruptura.API.Resources.SharedResources", typeof(SharedResources).Assembly);

    public static IEnumerable<object[]> ContentErrorCodes() =>
        typeof(ErrorCodes.Content)
            .GetFields(BindingFlags.Public | BindingFlags.Static)
            .Where(f => f is { IsLiteral: true, IsInitOnly: false } && f.FieldType == typeof(string))
            .Select(f => new object[] { (string)f.GetValue(null)! });

    [Theory]
    [MemberData(nameof(ContentErrorCodes))]
    public void EveryContentErrorCode_Resolves_InEnglishAndPortuguese(string code)
    {
