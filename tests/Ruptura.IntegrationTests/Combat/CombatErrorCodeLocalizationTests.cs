...
// ErrorCodes.Combat must resolve to a real value in BOTH the English (neutral) and pt-BR resources.
// Mirrors RewardErrorCodeLocalizationTests — a ResourceManager bound to the SharedResources embedded
...
public class CombatErrorCodeLocalizationTests
{
    private static readonly ResourceManager Resources =
        new("Ruptura.API.Resources.SharedResources", typeof(SharedResources).Assembly);

    public static IEnumerable<object[]> CombatErrorCodes() =>
        typeof(ErrorCodes.Combat)
            .GetFields(BindingFlags.Public | BindingFlags.Static)
            .Where(f => f is { IsLiteral: true, IsInitOnly: false } && f.FieldType == typeof(string))
            .Select(f => new object[] { (string)f.GetValue(null)! });

    [Theory]
    [MemberData(nameof(CombatErrorCodes))]
    public void EveryCombatErrorCode_Resolves_InEnglishAndPortuguese(string code)
    {
