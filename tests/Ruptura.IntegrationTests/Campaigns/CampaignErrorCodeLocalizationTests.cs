...
// ErrorCodes.Campaign must resolve to a real value in BOTH the English (neutral) and pt-BR resources.
...
public class CampaignErrorCodeLocalizationTests
{
    private static readonly ResourceManager Resources =
        new("Ruptura.API.Resources.SharedResources", typeof(SharedResources).Assembly);

    public static IEnumerable<object[]> CampaignErrorCodes() =>
        typeof(ErrorCodes.Campaign)
            .GetFields(BindingFlags.Public | BindingFlags.Static)
            .Where(f => f is { IsLiteral: true, IsInitOnly: false } && f.FieldType == typeof(string))
            .Select(f => new object[] { (string)f.GetValue(null)! });

    [Theory]
    [MemberData(nameof(CampaignErrorCodes))]
    public void EveryCampaignErrorCode_Resolves_InEnglishAndPortuguese(string code)
    {
