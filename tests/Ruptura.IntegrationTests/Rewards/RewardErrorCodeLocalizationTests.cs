...
// ErrorCodes.Reward must resolve to a real value in BOTH the English (neutral) and pt-BR resources.
// Mirrors EncounterErrorCodeLocalizationTests — a ResourceManager bound to the SharedResources embedded
...
public class RewardErrorCodeLocalizationTests
{
    private static readonly ResourceManager Resources =
        new("Ruptura.API.Resources.SharedResources", typeof(SharedResources).Assembly);

    public static IEnumerable<object[]> RewardErrorCodes() =>
        typeof(ErrorCodes.Reward)
            .GetFields(BindingFlags.Public | BindingFlags.Static)
            .Where(f => f is { IsLiteral: true, IsInitOnly: false } && f.FieldType == typeof(string))
            .Select(f => new object[] { (string)f.GetValue(null)! });

    [Theory]
    [MemberData(nameof(RewardErrorCodes))]
    public void EveryRewardErrorCode_Resolves_InEnglishAndPortuguese(string code)
    {
