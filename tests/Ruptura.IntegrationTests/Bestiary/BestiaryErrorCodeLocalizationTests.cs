...
// ErrorCodes.Bestiary must resolve to a real value in BOTH the English (neutral) and pt-BR resources.
// Mirrors GuildErrorCodeLocalizationTests — a ResourceManager bound to the SharedResources embedded
...
public class BestiaryErrorCodeLocalizationTests
{
    private static readonly ResourceManager Resources =
        new("Ruptura.API.Resources.SharedResources", typeof(SharedResources).Assembly);

    public static IEnumerable<object[]> BestiaryErrorCodes() =>
        typeof(ErrorCodes.Bestiary)
            .GetFields(BindingFlags.Public | BindingFlags.Static)
            .Where(f => f is { IsLiteral: true, IsInitOnly: false } && f.FieldType == typeof(string))
            .Select(f => new object[] { (string)f.GetValue(null)! });

    [Theory]
    [MemberData(nameof(BestiaryErrorCodes))]
    public void EveryBestiaryErrorCode_Resolves_InEnglishAndPortuguese(string code)
    {
